...
//        };
console.log('debut du traitemennt', new Date());
...
console.log('fin initialisation des tableau', new Date());
...
/* $(document).ajaxSuccess(function (event, xhr, settings) {
	...
	console.log('fin AJAJX datelastsave', new Date());
...
map.getView().on('propertychange', function (e) {
	...
	console.log('fin mapgetview on property change', new Date());
});
...
cartecomplete = new ol.layer.Vector({
				...
				console.log('fin ajout layer Wargaming avec geojson telecharger', new Date());
				layers = map.getLayers().getArray();
				vector = getLayerwarg(layers, "wargaming");
				varlayersource = vector.getSource();
				varlayersource.change();
...
function ModeAffichage(mode) {
	...
	setTimeout(function () {
		preloader.show();
		console.log('debut fonction mode affichage', new Date());
		switch (mode) {
		case 'Clan':
			effacericone();
			effacerbatailles();
		...
		console.log('fin fonction mode affichage', new Date());
		preloader.hide();
	}, 100);
...
console.log('debut de load log season', new Date());
	...
	$(document).ready(function () {
		...
		seasonTable.draw();
		console.log('fin charger load log season', new Date());
	});
}
...
function showLogClan() {
	...
	console.log('debut fonction table clan', new Date());
	$(document).ready(function () {
		...
		console.log('fin fonction afficher table clan', new Date());
		return true;
	});
}

function loadLogClan() {
console.log('debut fonction chargement tableau clan', new Date());
	// load the table Clan
		var tabevent = [];
...
console.log('debut fonction chrger tableau province', new Date());
...
console.log('debut fonction charger provvince', new Date());
...
$(document).ready(function () {
console.log('debut traitement termplate', new Date());
	/***************** Navbar-Collapse ******************/
